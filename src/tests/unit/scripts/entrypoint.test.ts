...
describe('entrypoint', () => {
  ...
  describe('LanguageStore', () => {
    it('initializes language from path locale if supported', () => {
      ...
      globalThis.location.pathname = `${base}/id/about/`;

      langStore.init();
      expect(langStore.lang).toBe('id');
      expect(globalThis.localStorage.setItem).not.toHaveBeenCalled();
    });
    ...
    it('sets language', () => {
      ...
      langStore.setLang('id');
      expect(langStore.lang).toBe('id');
      expect(globalThis.localStorage.setItem).toHaveBeenCalledWith('language', 'id');
    });
  });
});
